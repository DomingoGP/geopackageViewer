...
Designed for Delphi 6+ and Freepascal, Unicode support for Delphi 2009+
  ...
  Add function by Domingo Galmés 5-julio-2017
  : Read field from current row as blob to memory stream.
   procedure FieldAsBlobToStream(I: cardinal;aStream:TStream);
...
type
  ...
  public
    ...
    {: Mira si la tabla existe, añadida Domingo Galmes}
  ...
  public
    ...
    {: Read field from current row as blob to memory stream. Afegit Domingo Galmes}
    procedure FieldAsBlobToStream(I: cardinal;aStream:TStream);
    {: Read field from current row as blob to memory stream.}
...
begin
  if value then
    ...
    v := 0;
{$IFNDEF DARWIN} //todo  aclarir perque no troba la funcio.
...
procedure TSQLiteTable.FieldAsBlobToStream(I: cardinal;aStream:TStream);
var
  iNumBytes: integer;
  ptr: pointer;
begin
  ...
  begin
    ...
    aStream.write(ptr^, iNumBytes);
    //aStream.writebuffer(ptr^, iNumBytes);
    aStream.Position := 0;
  end;
end;
